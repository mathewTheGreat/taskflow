    ...
    <div>
          ...
          <div className="detail-create-body">
            ...
            <div className="detail-create-row">
              <div>
                ...
                <select value={newTaskPriority} onChange={e => setNewTaskPriority(e.target.value as TaskPriority)} className="detail-task-select">
              ...
              <div>
                ...
                <input type="date" value={newTaskDueDate} onChange={e => setNewTaskDueDate(e.target.value)} className="detail-task-select" />
              </div>
              <div>
                <label className="detail-create-label">Parent Task</label>
                <select value={newTaskParentId} onChange={e => setNewTaskParentId(e.target.value)} className="detail-task-select">
      ...
      {activeTab === 'board' && (
        <div className="detail-board">
          {(['pending', 'in_progress', 'completed'] as TaskStatus[]).map(status => (
            <div key={status} className="detail-board__column">
              <div className="detail-board__header">
                <StatusPill status={status} />
                <span className="detail-board__count">({groupedTasks[status].length})</span>
              </div>
              <div className="detail-board__cards">
                ...
                {groupedTasks[status].length === 0 && (
                  <p className="detail-board__empty">No tasks</p>
                )}
              </div>
            </div>
          ))}
      ...
      {activeTab === 'calendar' && (
        <Card className="placeholder-page"><p className="placeholder-page__text">Calendar view coming in Phase 2</p></Card>
      )}

      {activeTab === 'files' && (
        <Card className="placeholder-page"><p className="placeholder-page__text">File management coming in Phase 2</p></Card>
      )}
  ...
  return (
    <Card padding="none" className="task-group">
      <div className="task-group__header">
        ...
        <button className="task-group__actions"><IconDots /></button>
      </div>
      ...
      {expanded && (
        <>
            ...
            <table className="task-table">
              <thead>
                <tr className="task-table__header-row">
                  ...
                  <th className="task-table__header-cell task-table__header-cell--due">Due</th>
              ...
              <tbody>
                {tasks.map(task => {
                  ...
                  const completedSub = subtasks.filter(st => st.status === 'completed').length
                  const progress = subtasks.length > 0 ? Math.round((completedSub / subtasks.length) * 100) : (task.status === 'completed' ? 100 : task.status === 'in_progress' ? 50 : 0)

                  return (
                    <tr key={task.id} className="task-row">
                      <td className="task-row__cell">
                        <div className="task-title">
                          <button onClick={() => { const n: Record<TaskStatus, TaskStatus> = { pending: 'in_progress', in_progress: 'completed', completed: 'pending' }; onStatusChange(task.id, n[task.status]) }} className="task-title__checkbox">
                            {task.status === 'completed' ? <IconCheckCircle className="task-title__checkbox--completed" /> : task.status === 'in_progress' ? <IconCircleHalf className="task-title__checkbox--in_progress" /> : <IconCircle />}
                          </button>
                          <button onClick={() => onSelect(task)} className="task-title__text-btn">
                            <span className={`task-title__text${task.status === 'completed' ? ' task-title__text--completed' : ''}`}>
                              {task.title}
                            </span>
                            {subtasks.length > 0 && <span className="task-title__subtasks">({completedSub}/{subtasks.length})</span>}
                          </button>
                        </div>
                      </td>
                      <td className="task-row__cell">
                      ...
                      <td className="task-row__cell">
                        <select value={task.priority} onChange={e => onPriorityChange(task.id, e.target.value as TaskPriority)} className="detail-task-select detail-task-select--sm">
                          <option value="low">Low</option>
                      ...
                      <td className="task-row__cell">
                        {task.assignee_name ? <Avatar name={task.assignee_name} size="sm" /> : <span className="task-assignee--empty">Unassigned</span>}
                      </td>
                      <td className="task-row__cell">
                        <div className="task-progress">
                          <div className="task-progress__bar"><div className="task-progress__fill" style={{ width: `${progress}%` }} /></div>
                          <span className="task-progress__text">{progress}%</span>
                        </div>
                      </td>
                      <td className="task-row__cell task-actions">
                        <button onClick={() => onEdit(task)} className="task-action-btn"><IconEdit /></button>
                        <button onClick={() => onDelete(task.id)} className="task-action-btn task-action-btn--danger"><IconTrash /></button>
                      </td>
                    </tr>
                  )
                })}
          ...
          <div onClick={() => { setNewTaskParentId(''); setShowCreateTask(true); }} className="task-group__add">
            <IconPlus /> Add Task
          </div>
        </>
      )}
