                  ...
                  <tr key={task.id} className="task-row">
                    <td className="task-row__cell">
                      ...
                      {childTasks.filter(ct => ct.parent_id === task.id).length > 0 && (
                        <span className="task-title__subtasks">
                          {childTasks.filter(ct => ct.parent_id === task.id && ct.status === 'completed').length}/
                          {childTasks.filter(ct => ct.parent_id === task.id).length}
                        </span>
                      )}
                    </td>
                    <td className="task-row__cell">
...
function IconCalendar({ size = '15' }: { size?: string }) { return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> }
